...
function MovieInfoServiceClient:GetMoviesByName(title)
  self:send_GetMoviesByName(title)
  return self:recv_GetMoviesByName(title)
end

function MovieInfoServiceClient:send_GetMoviesByName(title)
  self.oprot:writeMessageBegin('GetMoviesByName', TMessageType.CALL, self._seqid)
  local args = GetMoviesByName_args:new{}
  args.title = title
  args:write(self.oprot)
  self.oprot:writeMessageEnd()
  self.oprot.trans:flush()
...
function MovieInfoServiceClient:recv_GetMoviesByName(title)
  local fname, mtype, rseqid = self.iprot:readMessageBegin()
  if mtype == TMessageType.EXCEPTION then
    local x = TApplicationException:new{}
    x:read(self.iprot)
  ...
  local result = GetMoviesByName_result:new{}
  result:read(self.iprot)
  self.iprot:readMessageEnd()
  if result.success ~= nil then
    return result.success
...
function MovieInfoServiceProcessor:process_GetMoviesByName(seqid, iprot, oprot, server_ctx)
  local args = GetMoviesByName_args:new{}
  local reply_type = TMessageType.REPLY
  args:read(iprot)
  iprot:readMessageEnd()
  local result = GetMoviesByName_result:new{}
  local status, res = pcall(self.handler.GetMoviesByName, self.handler, args.title)
  if not status then
    reply_type = TMessageType.EXCEPTION
    result = TApplicationException:new{message = res}
  ...
  oprot:writeMessageBegin('GetMoviesByName', reply_type, seqid)
  result:write(oprot)
  oprot:writeMessageEnd()
  oprot.trans:flush()
  return status, res
...
GetMoviesByName_args = __TObject:new{
  title
}

function GetMoviesByName_args:read(iprot)
  iprot:readStructBegin()
  while true do
    local fname, ftype, fid = iprot:readFieldBegin()
    if ftype == TType.STOP then
...
function GetMoviesByName_args:write(oprot)
  oprot:writeStructBegin('GetMoviesByName_args')
  if self.title ~= nil then
    oprot:writeFieldBegin('title', TType.I64, 1)
    oprot:writeI64(self.title)
    oprot:writeFieldEnd()
  end
  oprot:writeFieldStop()
  oprot:writeStructEnd()
...
GetMoviesByName_result = __TObject:new{
  success,
  se
}

function GetMoviesByName_result:read(iprot)
  iprot:readStructBegin()
  while true do
    local fname, ftype, fid = iprot:readFieldBegin()
    if ftype == TType.STOP then
...
function GetMoviesByName_result:write(oprot)
  oprot:writeStructBegin('GetMoviesByName_result')
  if self.success ~= nil then
    oprot:writeFieldBegin('success', TType.LIST, 0)
    oprot:writeListBegin(TType.STRING, #self.success)
